#include "data_tamer/sinks/ros2_rt_publisher_sink.hpp"

namespace DataTamer
{
ROS2RTPublisherSink::ROS2RTPublisherSink(
    ...
    const std::string& topic_prefix)
{
  ...
  schema_publisher_rt = std::make_unique<RTSchemaPublisher>(schema_publisher_);
  data_publisher_ = node->create_publisher<data_tamer_msgs::msg::Snapshot>(
      topic_prefix + "/data", data_qos);
  data_publisher_rt = std::make_unique<RTSnapshotPublisher>(data_publisher_);
}

void ROS2RTPublisherSink::addChannel(const std::string& channel_name, const Schema& schema)
{
...
bool ROS2RTPublisherSink::storeSnapshot(const Snapshot& snapshot)
{
  ...
  if (schema_changed_)
  {
    ...
    if (schema_publisher_rt->trylock())
    {
      schema_publisher_rt->msg_ = msg;
      schema_publisher_rt->unlockAndPublish();
    }
  }
  //----------------------------------------
  ...
  if (data_publisher_rt->trylock())
  {
    data_publisher_rt->msg_ = data_msg_;
    data_publisher_rt->unlockAndPublish();
  }
  return true;
}
